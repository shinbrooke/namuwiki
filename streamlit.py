...
with st.expander("이론적 배경: 집단지성"):
    ...
    특히 나무위키 문서라는 집단기억체제를 바탕으로 통합수행을 만들어 낼 때 사용되는 협력지성과 다양성을 중심으로 나무위키 문서의 편집 양상을 살펴보았습니다.
    """)
    st.write(" ")
    st.markdown("**집단지성의 질**")
    st.markdown("""
...
st.markdown('''
"나무위키 기본 방침"에는 나무위키의 목적과 특징을 다음과 같이 서술하고 있습니다.
...
얼마나 다양한 참여자들이 얼마나 활발하게 참여하는지 알아볼 수 있는 척도입니다. 사용자 수가 많을수록 다양한 사용자가 활발히 참여한 것으로, 질적으로 보다 우수한 집단지성이 형성될 가능성이 높다고 추정할 수 있을 것입니다. 한편, 사용자별 편집 횟수의 경우 특정 사용자의 편집 횟수가 지나치게 높을 경우, 문서에 개인적인 편향이 혼재할 우려를 의심해볼 수 있습니다.\n
**2. 각 페이지별 편집 총 횟수**\n
전체적으로 논의가 얼마나 활발히 일어났는지 파악할 수 있는 척도입니다. 편집 횟수가 많을수록 보다 활발히 내용이 업데이트되고, 많은 사람들이 관심을 갖고 있는 주제라고 해석할 수 있을 것입니다.\n
**3. 페이지별 수정의 양상**: 추가와 삭제의 양상 및 시간에 따른 수정의 추이\n
특정 페이지의 전반적인 경향을 확인할 수 있는 척도입니다. 과도한 삭제의 흔적이 보일 경우 반달리즘이 일어나지는 않았는지, 혹은 특정 사용자가 잘못된 내용을 다량 입력하여 혼란을 초래하지는 않았는지 알아볼 수 있을 것입니다. 또한, 시간이 흐름에 따라 내용의 추가 및 삭제가 줄어드는 경향이 보인다면 문서가 점차 안정화되었다고 판단할 수 있을 것입니다.\n
...
각 분야에서 랜덤으로 키워드를 10개씩 뽑아서 먼저 분석했습니다. 그 후, 더 많은 데이터를 수집하여 전체적인 경향성을 확인했습니다. 
...
\n우선 학문 분야에서는 '분류:수학 용어', '분류:물리학', '분류:생물학'에 속하는 모든 문서의 역사를 크롤링했습니다.
\n대중문화 분야에서는 우선 영화와 드라마 문서를 수집하기 위해 '분류:2016년 영화'부터 '분류:2019년 영화'까지의 문서와, '분류:넷플릭스 오리지널 드라마' 문서를 모두 수집한 뒤 랜덤으로 45개를 뽑았습니다. 
다음으로 스포츠 관련 문서도 대중문화에 포함하기 위해 '분류:축구 용어'와 '분류:야구 용어'의 모든 문서를 크롤링했습니다. 
추가로, 아이돌 관련 문서를 크롤링하기 위해 '걸그룹/목록'과 '보이그룹/목록' 문서에서 2011년부터 2016년까지의 걸그룹과 보이그룹에 대한 문서를 크롤링했습니다. 
\n수집된 학문 분야 문서의 수가 대중문화 분야 문서 수의 2배가 넘었기에, 학문 분야 문서는 랜덤으로 500개만 선별하여 사용했습니다. 대중문화 분야 문서는 총 379개를 사용했습니다.''')

#3. 데이터 분석 1: 10개씩 페이지 대조
st.markdown("<hr>", unsafe_allow_html=True)
title("데이터 분석 결과 1: 페이지 10개씩 살펴보기")
...
- 좀 더 많은 데이터를 살펴보아야겠지만, 대분류(대중매체/학문)에 따라 양상이 구분된다기보다는, **1. '게임, 드라마' 분야와 같이 인기 있고 최신 동향이 자주 업데이트되는지** 혹은 **2. '역사' 분야와 같이 사람마다 해석이나 의견이 달라 토론이 필요한지**에 따라 나뉘는 것으로 보입니다.
- 일반적인 개념을 서술하거나 이미 내용이 정립된 *(내용 보충 외의 별다른 조치가 필요하지 않은/논란이 없는/더 이상 활발히 연구되고 있지 않은)* 학술 용어 등은 편집자 수도, 편집 정도도 적었습니다.
- 편집자 수가 많은 경우, 추가되는 내용도 많기 때문에 문서의 내용도 더 충실할 경향이 있을 것으로 보입니다. 그렇지만 그만큼 몇몇 특정 사용자가 편향된 내용을 방대한 양으로 작성한다든지, 반달리즘이 일어날 위험도 상대적으로 크다고 볼 수 있습니다.
""")
st.write(" ")
...
- 사용자별 편집 횟수의 경우, x축 영역이 넓을수록 편집에 참여한 사용자가 많고, y값이 클수록 한 사용자가 여러 번, 집중적으로 편집했다고 볼 수 있습니다.
- 편집자가 많을수록 다양한 사람이 편집에 골고루 참여하는 것이라고 단정짓기는 어려워 보입니다. 예컨대 '임진왜란' 키워드를 보면 특정 몇 명이 100번 넘게 수정하고 있는 것을 확인할 수 있습니다. 보통 문서 편집이 활발하지 않을수록 편향성이 심할 것이라고 생각하기 쉬운데, 편집이 활발한 문서이더라도 특정 소수 편집자들의 편향이 크게 들어갈 우려가 있음에 유의해야 합니다.
- 요컨대, 편집자 수와 편집 횟수는 어느 정도 비례하는 것으로 보입니다. 두 횟수 모두 적은 경우는 상대적으로 내용이 빈약할 수밖에 없습니다. 물론 소수 편집자가 방대한 양을 작성할 수 있으나, 정보가 편향될 수 있음에 유의해야 할 것입니다. 반면 수정이 빈번하게 이루어지는 키워드는 내용이 상대적으로 충실해질 것이나, 편집자 수가 많아지는 만큼 특정 소수 편집자들에 의한 반달리즘에 취약하다고 볼 수 있을 것입니다. 특히 논란 있는 키워드일 경우 더욱 그러한 경향을 띤다고 할 수 있습니다.
""")
st.write(" ")
...
업데이트가 자주 되고, 다양한 의견을 낼 수 있는 키워드가 수정 횟수도 많고 편집에 참여하는 사용자 수도 많은 것으로 보입니다. 특히 '학문' 분야에서는 '임진왜란'과 같은 한국사 관련 페이지가 다른 페이지에 비해 월등히 높은 수정 횟수를 보여주고 있습니다. 이는 '임진왜란'이 활발한 토론의 장이 되고 있음을 시사하나, 이 토론이 반달리즘에 가까울지 건설적인 지식 구성에 가까울지는 데이터를 보다 자세히 살펴보아야 파악할 수 있을 것입니다.
""")
...
단순히 다양한 관점의 내용을 보충하는 것이 아니라, 다소 공격적인 어조로 왜곡된 정보에 대한 지적이 많이 이루어지는 것을 확인할 수 있습니다. 보다 정확한 고증을 위한 경우도 있겠으나, 자신의 주관적 의견을 내세우고 다른 의견을 무시하는 양상으로도 해석할 수 있을 것으로 보입니다.
""")
...
st.write("페이지별 변화한 글자수의 누적 합(cumulative sum)이 시간에 따라 어떻게 변화했는지 확인할 수 있는 그래프입니다.")
...
- 기울기가 수평에 가까울수록 글자수 변화가 적은 것이므로, 문서가 거의 수정되지 않았다고 해석할 수 있습니다. 예컨대 '게임서버'의 경우 문서의 큰 변화가 없는 것으로 보입니다.
- y값이 0에 가까울수록 문서 글자수가 적으므로, 문서 내용이 충실하지 못하다고도 해석할 수 있을 것입니다. 이 경우 키워드가 지엽적이라서 설명할 내용 자체가 상대적으로 적을 수도 있고, 역으로 글자수가 많다고 하여 충실한 문서라 해석할 순 없겠지만, 글자수 또한 문서 내용의 질에 기여하는 한 가지 지표이므로 이러한 방식으로 해석해볼 수 있을 것입니다.
...
st.write('나무위키의 편집 기록에는 편집한 사용자가 코멘트 항목에 해당 편집이 왜 이루어졌는지, 어떤 부분을 편집했는지 등을 자유롭게 작성할 수 있습니다.\n다만 이는 사용자가 원할 때 자의적으로 남기는 것으로서, 공란으로 비워두는 경우도 많습니다.\n우선 코멘트를 분석하여, 분야별로 지식의 수정이 어떻게 이루어지고 있는지 알아보겠습니다.')
...
- 그러나 '삭제'라는 키워드가 등장했다고 하여 반드시 지식의 축적이 덜하다고 볼 수는 없습니다. 대표적으로 맞춤법이나 문법을 지적하는 코멘트 같은 경우, '삭제'를 언급한다고 해서 지식 정보를 삭제한다고 보기 어렵습니다.
- 또한, 코멘트 작성은 의무가 아니므로 코멘트를 남기지 않는 사용자들이 많다는 점 역시 고려되어야 합니다.
- 지식이 어떻게 형성되고 있는지 더 정확한 모습을 알아보기 위해서는 구체적인 편집 기록을 직접 살펴볼 필요가 있습니다.
''')
st.write(" ")
...
st.write('''
나무위키에서 지식은 어떤 식으로 수정되고 있을까요? 조금 더 구체적으로, 지식이 추가되는 과정이 활발할까요, 삭제되는 과정이 활발할까요, 아니면 모두 비슷한 빈도로 일어날까요?
일반적으로 위키에서 어떤 문서의 글자 수가 늘어나면 지식이 추가된 것이고, 줄어들면 지식이 삭제된 것이라고 볼 수 있습니다. 물론 경향성이 그렇다는 것이지 형식논리적으로 그러하다는 뜻은 아닙니다. 이를테면 길고 장황한 표현을 너무 싫어하는 사용자가 있어서, 그 사용자가 수정을 할 때는 기존 문서의 표현을 간결하게 고쳐 쓴 뒤 자신의 기여분을 추가한다면 지식의 양이 늘어도 글자수는 줄어들 수 있습니다.
하지만 이러한 예외는 들어드린 예시에서 알 수 있듯 말 그대로 예외적인 것이고, 충분한 수의 문서와 수정 기록을 보면 그러한 예외는 그렇게 중요하게 고려하지 않아도 될 것입니다.
따라서 저희는 문서 글자 수의 변화 추이로 지식의 수정 양상을 간접적으로 확인하려 합니다.

이때 유의할 점은, 각 문서는 편집 기록의 양상이 다르다는 것입니다. 예컨대 어떤 문서는 150번 수정되었을 수 있고 어떤 문서는 500번 수정되었을 수 있습니다. 이를 보정하기 위해 문서가 처음 생성되었을 때부터 특정 스텝까지의 기록을 살펴보려고 합니다. 예를 들어 스텝이 100이라고 하면 (편집 횟수가 100회 이상인 문서에 한해) 첫번째 편집부터 100번째 편집까지만 고려하는 식으로 진행하였습니다.
...
아래는 총 스텝을 50으로 했을 때와 100으로 했을 때의 그래프입니다.
''')
...
- 그래프에서 0은 문서의 글자 수에 아무런 변화가 없었다는 것이고, 0보다 큰 것은 글자 수가 늘어났음을, 작은 것은 글자 수가 줄어들었음을 나타냅니다. 즉 그래프를 통해 0을 기준으로 지식이 전반적으로 추가되고 있는지, 혹은 삭제되고 있는지 볼 수 있습니다.
- 대중문화 관련 문서는 그래프가 전반적으로 0 위에 있으므로, 편집이 전반적으로 양의 방향으로 일어나고 있다고 할 수 있습니다.
- 학문 관련 문서는 그래프가 0 아래에 있는 경우도 적지 않습니다. 누군가가 추가한 지식을 (어떤 이유든지 간에) 다른 사람이 삭제하는 양상이 상대적으로 자주 관찰된다고 볼 수 있습니다.
- 그래프가 똑같이 0 위를 지나갈 때도, 대중문화 관련 문서는 학문 관련 문서보다 상대적으로 위쪽에 그래프가 그려지는 것을 확인할 수 있습니다. 이는 편집 양상이 지식이 추가되는 방식일 때 대중문화 관련 문서가 학문 관련 문서보다 더 많은 양의 정보가 추가된다는 뜻입니다.
- 시간이 지남에 따라, 문서 수정이 처음에 비해 상대적으로 적어짐을 확인할 수 있습니다. 즉 문서가 안정화되는 모습을 볼 수 있습니다.
''')
...
st.markdown("""
1. 나무위키를 통해 효과적인 집단지성의 구축이 가능합니다. 다만, 활발한 축적 방식의 토론을 기반으로 할 때 질 높은 집단지성을 형성할 수 있을 것으로 보입니다.
    - 많은 수의 참여자들이 글자를 추가하는 방향으로 편집하는 것을 확인할 수 있었습니다. 이는 다양한 사람들의 참여를 통한 축적이 이루어짐을 시사하는 듯합니다. 다만 그 과정에서 반달리즘이 일어나기도 했으므로, 축적 방식의 토론에 대한 강조가 필요해 보입니다.
    - 편집 과정에서 축적이 이루어지는 것을 통해, 협력지성이 발휘되어 구성원들이 집단 지성에 기여했다고 볼 수 있습니다.
    - 페이지별로 편집의 누적 합(cumulative sum)을 확인했을 때, 어느 정도 시간이 지난 후에는 수정이 줄어들면서 문서가 안정화되는 모습을 보여주었습니다. 이는 어느 정도 합의된, 질이 높은 집단 지성의 구축을 의미한다고 볼 수 있습니다.
""")
...
st.markdown("""3. 나무위키 플랫폼에서 '토론' 및 '역사'를 확인할 수 있는 기능을 제공하고 있지만, 대부분의 사용자들은 이를 따로 확인하지 않은 채 페이지상의 정보에 그대로 노출됩니다. 나무위키에서 지식을 습득할 때 편향되거나 근거가 불분명한 정보가 있을 수 있음을 자각하고, 개개인이 지식 습득에 있어서의 분별력을 기를 필요가 있을 것으로 보입니다. 한편, 보다 객관적이고 신뢰성 있는 정보를 전달하기 위해 각주에 출처를 명확히 표기하는 등의 방법을 활성화할 수 있는 방안을 고려할 필요도 있을 것입니다.""")
st.markdown("""4. 교육에서의 활용: 교육에서 나무위키와 같은 위키를 활용하여(올바른 활용 예시를 모델링하여), 개별적/우발적 학습 시의  가이드라인을 제공할 수 있을 것으로 보입니다.""")
