...
#title styling
def title(text):
     st.markdown(f'<p style="background-color:#3a8660;color:#ffffff;font-size:50px;border-radius:5%;">{text}</p>', unsafe_allow_html=True)
...
with st.expander("문제의식 설명"):
    ...
    st.write("""우리가 이렇게 일상적으로 사용하는 나무위키는 의도적으로든, 비의도적으로든 학습의 장이 되기도 합니다. 위키는 모르는 정보를 검색할 때, 인터넷 서핑을 하다가 새로운 정보가 눈에 띄었을 때 등 우발적이고 무의식적으로 학습이 쉽게 일어날 수 있는 장입니다. 그러나 누구나 글을 작성하고 수정할 수 있기 때문에, 신뢰성 측면에서 비판을 받기도 합니다. 믿을 수 없는 정보, 편향된 정보가 작성될 가능성이 높기 때문입니다. 실제로 나무위키 내에서도 '나무위키/비판 및 문제점'이라는 페이지에 이러한 측면이 서술될 만큼, 편향과 주관적 의견 서술의 측면이 문제시되고 있기도 합니다.""")
    st.write("""**3. 나무위키의 가능성과 한계**""")
...
""")
st.markdown("""[참고] 임진왜란 키워드의 코멘트는 아래에서 확인할 수 있습니다.""")
